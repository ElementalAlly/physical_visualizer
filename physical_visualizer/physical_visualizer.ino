...
void loop() { // Tick all motors, and tick the sampler. If the FFT has been executed, find the peaks in various ranges that scale exponentially, then sets the motors targets to the adjusted peaks.
  ...
  if (status == FFT) {
    ...
    int targets[7];
    int maxIndex[7];
    double factors[7] = {(double)maxTicks / 3000.0, (double)maxTicks / 8000.0, (double)maxTicks / 8000.0, (double)maxTicks / 6000.0, (double)maxTicks / 4000.0, (double)maxTicks / 3000.0, (double)maxTicks / 2000.0};
